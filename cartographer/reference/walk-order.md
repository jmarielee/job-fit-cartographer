...
**Labels → Math → Guards → Verdict → Display**

A label is assigned by a model. The math turns labels into a number. The
guards limit what that number is permitted to be. The verdict turns the
number into words. The display shows it.

Nothing in the file structure says this. It is the single most useful thing
a cold reader can be told, so the catalog is built on it.
